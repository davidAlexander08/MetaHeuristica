...
def executa_solucao_pl(sistema):
    ...
    # Generation limits: p <= max * u and p >= min * u
    ...
    # ---------------------------
    # Prepare outputs and CSV
    # ---------------------------
    rows = []
    for unit in sistema.geradores:
        for t in range(sistema.n_estagios):
            val_p = value(p[(unit.nome, t)]) if value(p[(unit.nome, t)]) is not None else 0.0
            rows.append({"Periodo": t, "Unidade": unit, "P(MW)": val_p})

    df_out = pd.DataFrame(rows)
    df_out.to_csv("Operacao-Despacho.csv", sep=";", index=False)
    print("Saved Operacao-Despacho.csv")
    ...
    df_table.to_csv("Tabela_Geracao_p.csv", sep=";", index=False)