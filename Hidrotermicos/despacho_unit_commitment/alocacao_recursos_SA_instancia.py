...
UnitCommitment = {}
Demanda = []
P = list(range(1, json_data["instance"] + 1))
custo_deficit = json_data["deficit"]
# Loop over buses
for bus in json_data["buses"]:
    print("Bus name:", bus["bus"])
    print("Load:", bus["load"])
    Demanda.extend(bus["load"])  # append all loads to Demanda

    # Loop over thermal units
    for unit in bus["thermal_units"]:
        UnitCommitment[unit["nome"]] = np.zeros(len(P))
        Terms.append(unit["nome"])
        Custo[unit["nome"]] = unit["CVU"]
        LimSup[unit["nome"]] = unit["max_power"]
        LimInf[unit["nome"]] = unit["min_power"]
        TON[unit["nome"]] = unit["min_uptime"]
        Toff[unit["nome"]] = unit["min_downtime"]

        print("Unit name:", unit["nome"])
        print(" CVU:", unit["CVU"])
        print(" max_power:", unit["max_power"])
        print(" min_power:", unit["min_power"])
        print(" min_uptime:", unit["min_uptime"])
        print(" min_downtime:", unit["min_downtime"])
        print(" ramp_up_limit:", unit["ramp_up_limit"])
        print(" ramp_down_limit:", unit["ramp_down_limit"])
        print(" initial_power:", unit["initial_power"])
        print(" initial_status:", unit["initial_status"])
        ...
        if(len(lista_usinas_ligadas_periodo) != 0 and len(lista_usinas_desligadas_periodo) != 0):
            ...
            for i in lista_consecutivos:
                ...
                while capacidade_instalada_periodo < Demanda[i]:
                    ...
                    #print("t: ", t, " unit_a_ser_ligada: ", unit_a_ser_ligada, " unit_a_ser_desligada: " ,unit_a_ser_desligada)
...
def simulated_annealing(T0=100.0, alpha=0.9, n_iter=100, Tf = 70):
        ...
        for iter in range(n_iter):
            ...
            if delta_fob < 0:
                ...
                if cost_curr < cost_best:
                    print("T: ", T, " iter: ", iter, " custo: ", total_cost(p_curr))
                    print("##############################")
                    print("iter: ", iter)
                    print("p_new: ", p_new)
...
df_solucao = pd.DataFrame(p_sol)
df_solucao['Demanda'] = Demanda
print(df_solucao)
